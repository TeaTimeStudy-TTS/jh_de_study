...
default_args = {
  ...
  'retries' : 1,                       # 작업 실패시 재시도 1회 자동 실행
  'retry_delay' : timedelta(minutes=5) # 작업 실패 후 5분후에 재시도 
  # 시나리오 : 작업 성공 -> 완료
  # 시나리오 : 작업 실패 -> 5분후 -> 1회 재시도 -> 성공 -> 완료 
  # 시나리오 : 작업 실패 -> 5분후 -> 1회 재시도 -> 실패 -> 완료(실패) 
  # --> 향후 일정에서 성공하더라도 실패 데이터 소급x, 이번 주기에 획득할/처리등 데이터등 포기
}
...
# 3. DAG 세션 오픈 
# 3.1 Operator 구성
  # 오퍼레이터 객체를 생성 -> Task가 정의됨 -> 구동이되면 -> Task Instance 
  # BashOperator를 통해서 작업(bash)을 3개 정의
  t1 = BashOperator(
    # id 구성값 : 영문자, 숫자, 하이픈, 마침표, 언더바만으로 구성
    # airflow의 지휘하에 작동되는 DAG 구성시 실제 할일을 구성하는 task 구분값
    # 작동확인 -> 로그
    task_id = 'date-print',
  ...
  t3 = BashOperator(
    task_id = 'echo-print',
    bash_command = 'echo "hello airflow task"' # 메시지 출력
  )
  # 시퀀스(구동 순서)
  # 3-2. 의존성 정의
  # t1 실행 -> t2 실행 -> t3 실행
